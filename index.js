...
function customAssign(...objects) {
  if (objects.length < 2) {return "Error: at least two objects needed."}
    const objMerge = Object.assign(...objects)
    const objNew = {}
    for (const a in objMerge) {
        if (typeof objMerge[a] === 'number') {
            objNew[a] = objMerge[a].toString()
        } else {
            objNew[a] = objMerge[a]
        }
    }
...
function customEntries(obj) {
  if (typeof obj !== 'object') {return 'Error: argument is not an object'}
    const arrValues = Object.entries(obj)
    return arrValues
...
function customObjectFromEntries(entries) {
  if (!Array.isArray(entries)) {return 'Error: argument is not an Array'}
  const objA = entries.map((el) => {
  ...
  // console.log(objA)
  return Object.fromEntries(objA)
...
function filterObjectsByKey(arr, key) {
  if (!Array.isArray(arr)) {return ['emptyArray']}
    const arrWithKey = arr.filter(el => {
 ...
 * Використовується метод Object.values() для отримання всіх значень об'єкта, 
 * а потім метод reduce() для обчислення суми значень.
 *
...
function getObjectValuesSum(obj) {
  ...
  return arrValues.reduce((acc, value) => acc + value)
...
function convertArrayToObj(arr) {
  ...
  for (let key in objA) {
    if (arr.flat(10).indexOf(key) !== arr.flat(10).lastIndexOf(key)) {
